...
class Quest (JQuest) :
 ...
 def onEvent (self,event,st) :
   ...
   if event == "7097-03.htm" :
     ...
     st.giveItems(PURIFIED_NECKLACE,1)
     st.set("cond","6")
     st.set("id","6")
     st.playSound("ItemSound.quest_middle")
   elif event == "7097-12.htm" :
     st.takeItems(PURIFIED_NECKLACE,1)
 ...
 def onTalk (self,npc,st):
   ...
   if id == CREATED :
     if st.getPlayer().getLevel() >= 3 :
       if st.getPlayer().getRace().ordinal() == 0 and st.getQuestItemsCount(MARK_OF_TRAVELER) > 0:
...
qnum  = 45
qdef  = str(qnum) + "_ToTalkingIsland"
qname = "To Talking Island"
...
STARTED.addQuestDrop(GALLADUCCI,PURIFIED_NECKLACE,1)
