...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,st):
   ...
   cond  = st.getInt("cond")
   id    = st.getState()
   count_AR = st.getQuestItemsCount(ARIELS_RECOMMENDATION)
   ...
   if id == CREATED :
     if st.getPlayer().getRace().ordinal() == 1 :
       if st.getPlayer().getLevel() >= 3 :
         ...
         htmltext = htmlhead + "Quest for characters level 3 above." + htmlfoot
         st.exitQuest(1)
     else :
   ...
   elif npcId == ARIEL and cond >=1 :
     if count_AR == 0 :
       htmltext = "7148-01.htm"
     else :
       htmltext = "7148-03.htm"
   elif npcId == ASTERIOS and cond == 2 and count_AR > 0 :
