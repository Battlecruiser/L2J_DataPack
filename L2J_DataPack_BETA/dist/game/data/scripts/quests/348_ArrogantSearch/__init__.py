...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,player):
    ...
    if npcId == HANELLIN :
        ...
        # Has returned before getting the powerstone
            ...
            lowbgrade = getRandom(10)+4103 #Low B grade weapon key materials
 ...
 def onAttack (self, npc, player, damage, isPet, skill):
     ...
     if npcId in ATTACK_DROPS_24.keys() :
         ...
         if st.getInt("cond") == cond and getRandom(1000) < ATTACK_DROPS_24[npcId][3] and st.getQuestItemsCount(ATTACK_DROPS_24[npcId][4]) > 0 :  # Attack drops are low chance
         ...
         if st.getInt("cond") == cond and getRandom(1000) < ATTACK_DROPS_25[npcId][3] and st.getQuestItemsCount(ATTACK_DROPS_25[npcId][4]) > 0 :  # Attack drops are low chance
 ...
 def onKill(self,npc,player,isPet):
     ...
     if npcId in DROPS.keys() :
         cond = DROPS[npcId][0]
         if st.getInt("cond") == cond and st.getQuestItemsCount(DROPS[npcId][1]) < DROPS[npcId][2] and getRandom(100) < DROPS[npcId][3] and (DROPS[npcId][4] == 0 or st.getQuestItemsCount(DROPS[npcId][4])>0) :
     ...
     if npcId in DROPS_29.keys() :
         ...
         if st.getInt("cond") == cond and st.getQuestItemsCount(DROPS_29[npcId][1]) < DROPS[npcId][2] and getRandom(100) < DROPS[npcId][3] and (DROPS[npcId][4] == 0 or st.getQuestItemsCount(DROPS[npcId][4])>0) :
