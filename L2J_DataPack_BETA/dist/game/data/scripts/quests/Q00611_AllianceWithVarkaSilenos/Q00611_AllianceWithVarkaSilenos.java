...
public class Q00611_AllianceWithVarkaSilenos extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			case "31378-18.html":
				...
				break;
			case "31378-21.html":
				if (!st.hasQuestItems(VALOR_FEATHER) || (getQuestItemsCount(player, KETRA_BADGE_SOLDIER) < SOLDIER_BADGE_COUNT[3]) || (getQuestItemsCount(player, KETRA_BADGE_OFFICER) < OFFICER_BADGE_COUNT[3]) || (getQuestItemsCount(player, KETRA_BADGE_CAPTAIN) < CAPTAIN_BADGE_COUNT[3]))
				{
					return getNoQuestMsg(player);
				}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if (member != null)
		{
			...
			{
				giveItems(killer, info.getItemId(), 1);
			}
		}
			...
			case State.STARTED:
				switch (st.getCond())
				{
					case 1:
						...
						break;
					case 2:
						htmltext = (st.hasQuestItems(VARKA_MARKS[0]) && (getQuestItemsCount(player, KETRA_BADGE_SOLDIER) >= SOLDIER_BADGE_COUNT[1]) && (getQuestItemsCount(player, KETRA_BADGE_OFFICER) >= OFFICER_BADGE_COUNT[1])) ? "31378-14.html" : "31378-13.html";
						break;
					case 3:
						htmltext = (st.hasQuestItems(VARKA_MARKS[1]) && (getQuestItemsCount(player, KETRA_BADGE_SOLDIER) >= SOLDIER_BADGE_COUNT[2]) && (getQuestItemsCount(player, KETRA_BADGE_OFFICER) >= OFFICER_BADGE_COUNT[2]) && (getQuestItemsCount(player, KETRA_BADGE_CAPTAIN) >= CAPTAIN_BADGE_COUNT[2])) ? "31378-17.html" : "31378-16.html";
						break;
					case 4:
						...
						break;
					case 5:
						if (!st.hasQuestItems(VARKA_MARKS[3]) || !st.hasQuestItems(WISDOM_FEATHER) || (getQuestItemsCount(player, KETRA_BADGE_SOLDIER) < SOLDIER_BADGE_COUNT[4]) || (getQuestItemsCount(player, KETRA_BADGE_OFFICER) < OFFICER_BADGE_COUNT[4]) || (getQuestItemsCount(player, KETRA_BADGE_CAPTAIN) < CAPTAIN_BADGE_COUNT[4]))
						{
