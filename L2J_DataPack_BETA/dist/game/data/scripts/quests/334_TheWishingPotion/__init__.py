...
#### WISH IV : I'd like to become the wisest person in the world.
     ...
     wish = st.getInt("wish")
     WISH_CHANCE = getRandom(100)
     if wish == 1 :
       if WISH_CHANCE <= 50:
         autochat(st.addSpawn(SUCCUBUS_OF_SEDUCTION,200000),MESSAGES[SUCCUBUS_OF_SEDUCTION][getRandom(len(MESSAGES))])
         autochat(st.addSpawn(SUCCUBUS_OF_SEDUCTION,200000),MESSAGES[SUCCUBUS_OF_SEDUCTION][getRandom(len(MESSAGES))])
         autochat(st.addSpawn(SUCCUBUS_OF_SEDUCTION,200000),MESSAGES[SUCCUBUS_OF_SEDUCTION][getRandom(len(MESSAGES))])
       else:
       ...
       if WISH_CHANCE <= 33 :
         autochat(st.addSpawn(GRIMA,200000),MESSAGES[GRIMA][getRandom(len(MESSAGES))])
         autochat(st.addSpawn(GRIMA,200000),MESSAGES[GRIMA][getRandom(len(MESSAGES))])
         autochat(st.addSpawn(GRIMA,200000),MESSAGES[GRIMA][getRandom(len(MESSAGES))])
       else :
       ...
       if WISH_CHANCE <= 33:
         st.giveItems(R1[getRandom(len(R1))],1)
         st.giveItems(R2[getRandom(len(R2))],1)
         st.giveItems(R3[getRandom(len(R3))],1)
         if not getRandom(3):
 ...
 def onTalk (self,npc,player):
   ...
   if npcId == TORAI and st.getQuestItemsCount(FORBIDDEN_LOVE_SCROLL) :
     ...
     st.giveItems(4409,1)
     st.giveItems(4408,1)
     htmltext = "30743-0"+str(getRandom(6)+1)+".htm"
     npc.deleteMe()
   elif npcId == RUPINA:
     if getRandom(100) <= 4:
       ...
       st.giveItems(R4[getRandom(len(R4))],1)
 ...
 def onKill(self,npc,player,isPet):
   ...
   if npcId == SECRET_KEEPER_TREE and cond == 1 and not st.getQuestItemsCount(SECRET_BOOK):
      ...
      if getRandom(100) <= chance and not st.getQuestItemsCount(item) :
         st.giveItems(item,1)
         if check_ingredients(st,1):
